						...
						elseif (ctype_alpha($_POST['pattern']) != true)
						{
						    $message = "Pattern must contain alphabet letters only.";
						}
						// verify that the last name is the correct length
						elseif (strlen( $_POST['pattern']) < 1)
						{
						    $message = 'Incorrect Length for pattern';
						}
						else 
						{
						...
						{
						$vari = mysqli_real_escape_string($con, $_POST['pattern']);
						// Insert data into mysql 
						$sql="INSERT IGNORE INTO PATTERN(Name) VALUES('$vari')";
